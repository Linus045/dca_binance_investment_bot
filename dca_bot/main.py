...
def main():
    ...
    order_fulfilled_checker_thread: OrderFulfilledChecker = OrderFulfilledChecker(
        order_list_manager,
        on_order_filled_callback=on_order_filled_callback,
        get_order_status_callback=get_order_status_callback,
    )
        ...
        while running:
            try:
                ...
                for investment_strategy in dca_investment_strategies:
                    ...
                    LOG_INFO(f"Current average price for {symbol}: {bot.get_avg_price(symbol)}")
                    LOG_INFO(f"Balance base asset {base_asset}: {bot.get_asset_balance(base_asset)}")
                    LOG_INFO(f"Balance quote asset {quote_asset}: {bot.get_asset_balance(quote_asset)}")
