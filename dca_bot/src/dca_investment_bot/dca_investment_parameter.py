...
class DCAInvestmentParameter:
    ...
    def parse_interval_to_seconds(self, interval):
        if interval == "1d":
            ...
            raise UnknownIntervalException(f'Unknown interval "{interval}"')
    ...
    def parse_investment_time_to_seconds(self, investment_time):
        try:
            ...
            raise InvalidParameterFormat(
                "Investment time uses invalid format. \
                    HH:MM expected but {} was given".format(
                    investment_time
                )
            )
    ...
    def date_to_seconds(self, date):
        try:
            ...
            raise InvalidParameterFormat(
                f"Start date uses invalid format. \
                    YYYY-MM-DD expected but {date} was given"
            )
